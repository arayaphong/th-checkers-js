...
เบี้ยก็กินต่อเนื่องได้เช่นกัน แต่ทุกทอดยังอยู่ภายใต้ **กติกาของเบี้ย** ล้วน ๆ คือ:
...
![การกินต่อเนื่องของเบี้ย](images/multi-capture-pion.svg)

> เบี้ยขาว `C8` กินหมากดำ `D7` ลง `E6` (จุด 1) แล้วหักเลี้ยวกินหมากดำ `D5` ลง `C4` (จุด 2) — กิน 2 ตัวในตาเดียว โดยทุกทอดยังเป็นการกินไปข้างหน้าทีละช่อง

### ฮอสกินต่อเนื่องแบบบินไกล

จุดเด่นของฮอสคือ **ทุกทอดของการกินต่อเนื่อง ฮอสจะร่อนข้ามช่องว่างไปได้ไม่จำกัดระยะ** ก่อนถึงหมากที่จะกิน ไม่ใช่เฉพาะทอดแรก — หลังจากลงจอดแล้วกินต่อ ก็ยังร่อนไกลได้อีกในทอดถัดไปเช่นกัน

ฮอสจะทำซ้ำแบบนี้ไปเรื่อย ๆ — **กิน → ลงจอด → มองหาตัวที่กินได้จากช่องที่ลงจอด** — กี่ทอดก็ได้ ไม่จำกัดจำนวน และ **สายการกินจะจบก็ต่อเมื่อจากช่องที่ลงจอดล่าสุดไม่มีหมากให้กินอีกในทุกทิศ**

ข้อควรระวัง 2 ข้อ:

- **"ไม่จำกัดระยะ" คือระยะ "ก่อนถึง" ตัวที่จะกิน** (ช่องว่างที่ร่อนข้ามไปหาเหยื่อ) แต่ **จุดที่ลงจอดยังต้องเป็นช่องว่างช่องแรกถัดจากตัวที่ถูกกินเสมอ** — กฎ "ลงติดตัวที่กิน" มีผลกับการกินทุกทอด เลือกลงไกลกว่านั้นไม่ได้
- **แต่ละทอดหักเลี้ยวเปลี่ยนทิศได้** ทอดถัดไปไม่จำเป็นต้องไปต่อในแนวทแยงเดิม

![การกินต่อเนื่องแบบบินไกลของฮอส](images/multi-capture-dame-fly.svg)

> ฮอสขาว `A8` ร่อนข้ามช่องว่าง `B7`, `C6` (วงประน้ำเงิน) ไปกินหมากดำ `D5` แล้วลงที่ `E4` (จุด 1) จากนั้น **หักเลี้ยว** ร่อนข้าม `D3` ไปกินหมากดำ `C2` แล้วลงที่ `B1` (จุด 2) — กิน 2 ตัวในตาเดียว โดยทอดแรกเป็นการบินไกลข้ามช่องว่าง 2 ช่อง แต่ทั้งสองทอดก็ยังลงจอดติดกับตัวที่กินเสมอ
...
- ถ้าเบี้ยเลื่อนขั้น **ระหว่างการกินต่อเนื่อง** สายการกินจะ **จบทันที** ที่ช่องเลื่อนขั้น (ไม่กินต่อในตานั้น แม้จะยังมีตัวให้กิน)
